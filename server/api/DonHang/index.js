...
module.exports = function(app) {
        ...
        try {
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    const {trangthai,id_kh} = req.body
                   // console.log("========================================================")
                   // //console.log(page,trangthai,id_kh)
                    ...
                    const newQuery = await pool.query(`
                        ...
                        ${id_kh === undefined ? '' : ` and don_hang.id_kh = ${id_kh}`}
    ...
    app.get("/TotalPageTatCa/KH/:id_kh", async(req,res)=>{
        try {
    ...
    app.post('/DonKhieuNai/:id_don' , async(req,res)=>{
        try {
            const {token} = req.body
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    console.log(id_don,ghi_chu)
                    const newQuery = await pool.query(`
                        update  don_hang set 
                        trangthai = N'Khiếu nại',
                        ghi_chu = N'${ghi_chu}'
                        where id_don = ${id_don}
                    `)
    ...
    app.post('/DonThanhCong/:id_don' , async(req,res)=>{
        try {
            const {token} = req.body
    ...
    //#region Khách hàng
    app.get(`/TotalPageDangXuLy/KH/:id_kh` , async(req,res)=>{
        try {
    ...
    app.get(`/TotalPageDaDatHang/KH/:id_kh` , async(req,res)=>{
        try {
            const {id_kh} = req.params
    ...
    app.get(`/TotalPageDangPhatGiaoVan/KH/:id_kh` , async(req,res)=>{
        try {
            const {id_kh} = req.params
            const newQuery = await pool.query(`
            select count(*) from don_hang
    ...
    app.get(`/TotalPageHangVeKho/KH/:id_kh` , async(req,res)=>{
        try {
            const {id_kh} = req.params
            const newQuery = await pool.query(`
            select count(*) from don_hang
    ...
    app.get(`/TotalPageKhieuNai/KH/:id_kh` , async(req,res)=>{
        try {
            const {id_kh} = req.params
            const newQuery = await pool.query(`
            select count(*) from don_hang
    ...
    app.get(`/TotalPageShopHuy/KH/:id_kh` , async(req,res)=>{
        try {
            const newQuery = await pool.query(`
        ...
        try {
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                //console.log(data)
                    ...
                    if(kiemtra_.rows[0].trangthai === "Đang xử lý" || kiemtra_.rows[0].trangthai === "Đã đặt hàng"){
                        let tgt =  parseInt(ti_gia_tinh.rows.map(x=>x.ti_gia_tinh))
    ...
    app.post("/XacNhanTrangThai/:id_don",async (req,res)=>{
        try {
            ...
            func.GuiThongBao(`[Đơn hàng] Chuyển trạng thái`,` Đã chuyển sang trạng thái ${stt}`,
        ...
        try {
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                // const {trangthai} = req.params
                    const {tim_theo_ten,
                        tim_theo_ma,
                        tim_theo_van,tu_ngay,den_ngay,trangthai,id_kh} = req.body

                ////  console.log(id_kh)
                    const newQuery = await pool.query(   `
                    select ten_kh,* from don_hang,khachhang
                    where don_hang.id_kh = khachhang.id_kh
                    ${id_kh  === null || id_kh  === '' || id_kh  === undefined ? "" : ` and don_hang.id_kh = ${id_kh}`}
                    ${tu_ngay === null || tu_ngay === ''  ? '' : `and ngay >= '${tu_ngay}'`}
                    ${den_ngay === null || den_ngay === '' ? '' : `and ngay <= '${den_ngay}'`}
                    ${tim_theo_ma === null || tim_theo_ma === '' ? '' : `and don_hang.ma_don like N'%${tim_theo_ma}%'`}
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    insert into don_hang(
                        ma_don,van_don,so_can,so_khoi,id_kh,ghi_chu,tien_can,tien_khoi,phi_noi_dia,phi_dich_vu,phu_phi,ngay
                    )
    ...
    app.get('/CapNhapVanDonChuyen/:id_don/:ma_don' , async(req,res)=>{
        try {
            const {id_don,ma_don} = req.params

            const newQuery = await pool.query(`
                update don_hang set van_don = N'${ma_don}',
                trangthai = N'Đang phát giao vận'
                where id_don = ${id_don}          
            `)

            res.json({
                status: newQuery.rowCount >= 0 ? 1 : 0,
                data: newQuery.rowCount >= 0 ? id_don : 0
            })
        } catch (error) {
