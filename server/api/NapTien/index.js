...
module.exports = function(app) {
    ...
    app.post("/NapTien" , async (req,res)=>{
        try {
        ...
        try {
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    // console.log(page)
                    const {tu_ngay,den_ngay,tim_theo_ten,tim_theo_ma,tim_theo_van,token, id_kh} = req.body
                    
                    const newQuery = await pool.query(`
                        select * from nap_tien,khachhang
                        where khachhang.id_kh = nap_tien.id_kh
                        ${id_kh === '' || id_kh === undefined || id_kh === null ? '' : ` and nap_tien.id_kh = ${id_kh}`}
                        ${tu_ngay === '' || tu_ngay === undefined || tu_ngay === null ? ''  : `and nap_tien.ngay_nap >= '${tu_ngay}'`} 
                        ${den_ngay === ''||  den_ngay === undefined || den_ngay === null ? ''  : `and nap_tien.ngay_nap <= '${den_ngay}'`} 
                        ${tim_theo_ten === '' ||  tim_theo_ten === undefined || tim_theo_ten === null ? ''  : `and LOWER(convertTVkdau(khachhang.ten_kh)) LIKE LOWER(convertTVkdau(N'%${tim_theo_ten}%'))`} 
                        order by nap_tien.ngay_nap DESC
    ...
    app.get("/TotalPageDSNapTienKH/:id_kh", async(req,res)=>{
        try {
            ...
            fs.readFile('TenShop.txt', 'utf8', async (err, data)=>{
                ...
                if(token.split("*")[1] === data && checkToken.rowCount > 0){
                    ...
                    const newQuery = await pool.query(`
                        ...
                        ${id_kh === undefined  ? "" : ` and nap_tien.id_kh = ${id_kh}`}
                        order by nap_tien.ngay_nap DESC
