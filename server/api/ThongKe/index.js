...
module.exports = function(app) {
        ...
        app.post(`/ThongKeTienDonHang/:page`, async(req, res) => {
                    try {
                        ...
                        SUM(
                            so_can*tien_can+so_khoi*tien_khoi
                        )"tien_can_khoi"
                        from don_hang,khachhang
                        where don_hang.id_kh = khachhang.id_kh
                        ${id_kh === undefined || id_kh === '' || id_kh === null ? "" : `and don_hang.id_kh = ${id_kh}`  }
                        ...
                        GROUP BY khachhang.ten_kh,don_hang.ngay, don_hang.ma_don,don_hang.van_don,don_hang.image
    ...
    app.get("/TotalPageThongKeTienDon", async(req,res)=>{
        try {
            const newQuery = await pool.query(`
                ...
                group by khachhang.ten_kh,don_hang.image
            `)  
            res.json({
    ...
    app.get("/TotalPageThongKeTienDonKH/:id_kh", async(req,res)=>{
        try {
