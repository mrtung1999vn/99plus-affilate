...
const GuiThongBao = async (tieu_de,noi_dung,ngay,loai_tb,id_don,image)=>{
        try{
            ...
            const image = await pool.query(`select image from don_hang_ct
                where id_don = ${id_don}
                limit 1
            `)
            console.log(image.rows)
            console.log(`
            insert into thong_bao(tieu_de,noi_dung,ngay,loai_tb,id_kh,id_don,image,trang_thai_xem,image)
            values(
                N'${tieu_de}',N'${noi_dung}','${ngay}',N'${loai_tb}',${khachhang.rows[0].id_kh},${id_don},false,N'${image.rows[0].image}'
            )
            `)

            const newQuery = await pool.query(`
            insert into thong_bao(tieu_de,noi_dung,ngay,loai_tb,id_kh,id_don,trang_thai_xem,image)
            values(
                N'${tieu_de}',N'${noi_dung}','${ngay}',N'${loai_tb}',${khachhang.rows[0].id_kh},${id_don},false,N'${image.rows[0].image}'
            )
        `)
